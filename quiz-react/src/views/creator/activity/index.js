...
import { getAllActivities } from "../../../services/creator/dashboard.service";
...
const Activity = () => {
  ...
  const loadData = async () => {
    try {
      let res = await getAllActivities();
      console.log(res);
  ...
  const formatActivitySentence = (activity) => {
    ...
    if (activity?.type === "Comment") {
      sentence = `On ${formattedDate}, new comment added on poll ${activity?.pollId} : "${activity?.comment}"`;
    } else if (activity?.type === "Poll") {
      sentence = `On ${formattedDate}, new poll created with the title "${activity?.title}"`;
    }
            ...
            <div class="rounded-t mb-0 px-0 border-0">
              ...
              <div class="block w-full">
                ...
                <ul class="my-1">
                  {recentActivity.today.length > 0 &&
                    recentActivity.today.map((activity, index) => (
                      <li class="flex px-4">
                        ...
                        ${
                          index >= recentActivity.today.length
                            ? "border-b border-gray-100 dark:border-gray-400"
                            : ""
                        } `}
                        >
                  ...
                  {recentActivity.today.length === 0 && (
                    <li class="flex px-4">
                      <div class="flex-grow flex items-center text-sm text-gray-600 dark:text-gray-100 py-2">
                        <div class="self-center">No recent activity</div>
                      </div>
                    </li>
                  )}
                  {/* <li class="flex px-4">
                ...
                <ul class="my-1">
                  {recentActivity.yesterday.length > 0 &&
                ...
                <ul class="my-1">
                  {recentActivity.older.length > 0 &&
                    recentActivity.older.map((activity) => (
                      <li class="flex px-4 h-16 items-center hover:bg-slate-900 border-b">
                        ...
                        {activity.type === "Poll" && (
                          <div class="w-9 h-9 rounded-full flex-shrink-0 bg-green-500 my-2 mr-3">
                            <svg
                              class="w-9 h-9 fill-current text-indigo-50 ml-2 mt-2.5"
                              viewBox="0 0 36 36"
                            >
