...
import {
  deletePolls,
  getAllPolls,
} from "../../../services/admin/poll.service";
...
const PollList = () => {
  ...
  const loadData = async () => {
    try {
      const response = await getAllPolls(page, limit);
  ...
  const handleDeletePoll = async (pollId) => {
    try {
      ...
      if (!confirm) return;
      const response = await deletePolls(pollId);
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="grid auto-cols-max justify-end grid-flow-col gap-3">
          ...
          <div className="relative">
            ...
            <div class="bg-slate-800 border border-slate-600 rounded absolute w-44 mt-2 text-md">
              <div class="text-md py-2">
                <button class="flex pl-4 items-center mb-1 text-indigo-400 w-full h-8 hover:bg-slate-900">
                  <svg class="mr-2" fill="currentColor" width="12" height="9">
          ...
          <Link class="text-white bg-indigo-500 inline-flex items-center px-6 py-1.5 rounded"
          ...
          >
            ...
            <span class="ml-1">Create Poll</span>
          </Link>
        </div>
      </div>
